...
export const getUserPost = async (req,res)=>{
    try {
        ...
        .populate({path:'author',select:'username,profilePicture'})
        .populate({path:'commemts',sort:{createdAt:-1},populate:{path:'author',select:'username,profilePicture'}});
        return res.status(200).json({
            posts,
            success:true
        })
...
export const getCommentsOfPost = async (req,res) =>{
    try {
        ...
        if(!comments) return re.status(404).json({message:'No comments',success:false});
...
export const bookmark = async (req,res) => {
    try {
        ...
        if(user.bookmarks.includes(post._id)){
            ...
            await user.save();
            return res.status(200).json({method:'unsaved',message:'Post removed from bookmark',success:true});
        }
        else{
            await user.updateOne({$addToSet:{bookmarks:post._id}});
            await user.save();
            return res.status(200).json({method:'saved',message:'Post bookmarked',success:true});
        }
